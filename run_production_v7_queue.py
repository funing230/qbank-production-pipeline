...
QWEN_TECH_FAILURE_SOURCES = {"exception", "http_error", "parse_error"}
...
def build_kp_queue(
    ...
    if subject_ids:
        ordered_subjects = [s for s in subject_ids if s in SUBJECTS]
    else:
...
def qwen_worker(worker_id: int, qwen_queue: Queue, image_queue: Queue, regen_queue: Queue, reviewer: QwenReviewer,
    ...
    while not stop_event.is_set() or not qwen_queue.empty():
        ...
        if not wait_until_enabled("qwen_enabled", f"qwen{worker_id}", stats, stats_lock):
            ...
            return
        try:
            result = reviewer.review(task.question.get("question_json", task.question), task.kp_info)
